...
```{r}
X <- read_csv(here('src', 'new_studies', 'processedData', 'chf_features.csv')) %>%
  rename(date=`...1`) %>% as.data.table() %>% mutate(date=ym(date)) %>% drop_na() %>%
  filter((date >= ymd('1999-07-01')) & (date <= ymd('2017-01-01')))
y <- read.csv(here('src', 'new_studies', 'processedData', 'chf_target.csv')) %>%
  rename(date=Date, EURCHF=M0_EUR1) %>% as.data.table() %>% mutate(date=ym(date)) %>% drop_na() %>%
  filter((date >= ymd('1999-07-01')) & (date <= ymd('2017-01-01')))
...
```{r}
formula_X <- paste0("`", paste(names(data_lag)[-grep("EURCHF|date", names(data))], collapse="` + `"), "`")
fomula <- paste(paste("EURCHF", "~", formula_X), " -1")
...
```{r}
# rolling regression
rolling_lm <- roll_lm(x = data_lag %>% select(EURCHF) %>% as.matrix(),
                      y = data_lag %>% select(-EURCHF, -date) %>% as.matrix(),
...
```{r}
X_selected <- X %>% select(date, `% foreign currency investment`, `chg EUR GDP`, `CPI YoY`)
melt_X_selected <- melt(X_selected, "date")
ggplot(melt_X_selected, aes(x = date, y = value, color = variable)) +
  geom_line() +
  theme_minimal() +
  labs(title = 'CHF features', x = 'Time', y = 'Feature')
```
...
```{r, warning=FALSE}
model_decoupled <- seqICP(X = X_selected %>% select(-date) %>% as.matrix(),
                          Y = y %>% select(-date) %>% as.matrix(),
                          par.model	= list(pknown = TRUE, p = 2),
                          model = "ar",
                          max.parents = 5,
                          stopIfEmpty=FALSE,
                          silent=FALSE,
                          test = "variance")
summary(model_decoupled)
```
