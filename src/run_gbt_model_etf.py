...
def gbt_cv_random_search(X_train, y_train, n_candidates=50, n_splits=5, random_state=42):
    """
    Random search over academic-standard GBT hyperparameter space
    with TimeSeriesSplit CV.

    Hyperparameter ranges follow:

    - Gu, Kelly & Xiu (2020, RFS):
        Shallow trees (max_depth 1-3), learning_rate in {0.01, 0.1},
        n_estimators up to 1000, subsample < 1 for stochastic boosting.

    - Bianchi, Büchner & Tamoni (2021, JFE):
        Emphasis on interaction depth 1-2 (stumps/pairs),
        shrinkage 0.001-0.1, early stopping via n_estimators.

    - Coulombe, Leroux, Stevanovic & Surprenant (2022, JoE):
        max_depth {1,2,3}, learning_rate {0.01, 0.05, 0.1},
        n_estimators {100, 500, 1000}, subsample {0.5, 0.8, 1.0}.

    - Medeiros, Vasconcelos, Veiga & Zilberman (2021, JBES):
        Conservative depth (1-4), moderate shrinkage,
        min_samples_leaf to prevent overfitting in low SNR.

    Key academic insight: financial return prediction has very low SNR,
    so the literature strongly favors:
      - Very shallow trees (depth 1-3, often stumps)
      - Strong shrinkage (low learning_rate)
      - Large number of boosting rounds with early-stopping-like selection
      - Stochastic boosting (subsample < 1) for variance reduction
    """
    param_grid = {
    ...
    # Validate max_features given dimensionality
    ...
    if best_params is None:
        # Fallback: conservative defaults from Gu et al. (2020)
        best_params = {
            'n_estimators': 500,
            'max_depth': 2,
            'learning_rate': 0.01,
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run Gradient Boosted Trees forecasting with TimeSeriesSplit CV")
    parser.add_argument("--target", type=str, default="SPY", help="Target variable to forecast")
    parser.add_argument("--pred_method", type=str, default="gbt-tscv", help="Prediction method name")
...
if __name__ == "__main__":
    ...
    for step in tqdm(
        ...
        try:
            ...
            # --------------------------------------------------
            # GBT CV: random search over academic hyperparameter space
            # --------------------------------------------------
            best_params = gbt_cv_random_search(
                X_train, y_train_arr,
            ...
            print(f"[{args.pred_method} step={step}] GBT fit/predict failed: {e}")
            continue
    ...
    if len(predictions) == 0:
        predictions_df = pd.DataFrame(columns=[
            ...
            "best_learning_rate", "best_subsample", "best_max_features",
            "best_min_samples_split", "best_min_samples_leaf", "n_features"
        ])
    else:
