...
def generate_glmnet_lambda_grid(X, y, n_lambda=200, lambda_min_ratio=1e-4):
    """
    Generate a lambda grid following glmnet's strategy for Lasso:
    - lambda_max = max(|X'y|) / n  (smallest lambda zeroing all coefficients)
    - lambda_min = lambda_min_ratio * lambda_max
    - log-spaced grid from lambda_max to lambda_min

    sklearn Lasso alpha = lambda_glmnet (no n scaling, since Lasso
    minimizes 1/(2n)||y-Xb||^2 + alpha * ||b||_1)
    """
    n = X.shape[0]
    lambda_max = np.max(np.abs(X.T @ y)) / n
    lambda_min = lambda_min_ratio * lambda_max

    alphas = np.logspace(np.log10(lambda_max), np.log10(lambda_min), num=n_lambda)

    return alphas


def generate_classic_logspace_grid(n_lambda=200, alpha_min=1e-6, alpha_max=1e2):
    """
    Generate a classical log-spaced grid of sklearn Lasso alphas.
    Range [1e-6, 1e2] covers typical Lasso regularization strengths.
    """
    alphas = np.logspace(np.log10(alpha_min), np.log10(alpha_max), num=n_lambda)
    return alphas


def lasso_cv_random_search(X_train, y_train, grid_strategy='glmnet',
                           n_candidates=50, n_splits=5, random_state=42,
                           fit_intercept=True):
    """
    Random search over a lambda/alpha grid with TimeSeriesSplit CV.

    grid_strategy:
        'glmnet'  - data-driven glmnet-style grid
        'classic' - fixed log-space grid [1e-6, 1e2]

    Returns the best sklearn Lasso alpha.
    """
    ...
    for alpha in candidate_alphas:
        ...
        for train_idx, val_idx in tscv.split(X_train):
            ...
            model = Lasso(alpha=alpha, fit_intercept=fit_intercept, max_iter=50000, tol=1e-4)
            model.fit(X_tr, y_tr)
...
def parse_pred_method(pred_method):
    ...
    Format: lasso-tscv-{grid}-{intercept_tag}
        grid:          'glmnet' | 'classic'
        intercept_tag: 'intercept' | 'nointercept'

    Examples:
        'lasso-tscv-glmnet-intercept'    → glmnet grid, fit_intercept=True
        'lasso-tscv-glmnet-nointercept'  → glmnet grid, fit_intercept=False
        'lasso-tscv-classic-intercept'   → classic grid, fit_intercept=True
        'lasso-tscv-classic-nointercept' → classic grid, fit_intercept=False
    """
    valid_methods = {
        'lasso-tscv-glmnet-intercept':    ('glmnet',  True),
        'lasso-tscv-glmnet-nointercept':  ('glmnet',  False),
        'lasso-tscv-classic-intercept':   ('classic',  True),
        'lasso-tscv-classic-nointercept': ('classic',  False),
    }
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run Lasso Regression forecasting with TimeSeriesSplit CV")
    parser.add_argument("--target", type=str, default="SPY", help="Target variable to forecast")
    parser.add_argument("--pred_method", type=str, default="lasso-tscv-glmnet-intercept", help="Prediction method name")
...
if __name__ == "__main__":
    ...
    for step in tqdm(
        ...
        try:
            ...
            # --------------------------------------------------
            # LASSO CV: random search over selected grid strategy
            # --------------------------------------------------
            best_alpha = lasso_cv_random_search(
                X_train, y_train,
            ...
            # FIT FINAL MODEL
            # --------------------------------------------------
            final_model = Lasso(alpha=best_alpha, fit_intercept=fit_intercept, max_iter=50000, tol=1e-4)
            ...
            n_nonzero = int(np.sum(np.abs(final_model.coef_) > 1e-10))
            ...
            print(f"[{args.pred_method} step={step}] Lasso fit/predict failed: {e}")
            continue
    ...
    if len(predictions) == 0:
        predictions_df = pd.DataFrame(columns=[
            ...
            "fit_intercept", "n_features", "n_nonzero_coefs"
        ])
    else:
